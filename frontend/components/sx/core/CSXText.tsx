...
const VARIANT_STYLES: Record<CSXTextVariant, string> = {
  ...
  body3Button: `${BOX_RESET} inline-flex items-center text-xs font-normal leading-none tracking-[-0.025em]`,
  cardPrice: `${BOX_RESET} text-[1.125rem] font-semibold leading-none tracking-[-0.02em]`,
  ...
  spaced: `${BOX_RESET} text-[0.625rem] font-normal uppercase leading-normal tracking-[0.06em]`,
  chartAnimatedPrice: `${BOX_RESET} text-[1.75rem] font-semibold leading-none tracking-[-0.02em] tabular-nums`,
}
...
export function CSXText({ variant, color, style, children, ...rest }: CSXTextProps) {
  const Tag = VARIANT_TAG[variant]

  return (
    <Tag
      className={"font-sans " + VARIANT_STYLES[variant]}
      style={{ ...(color ? { color: resolveSXColor(color) } : {}), ...style }}
      {...rest}
    >
      {children}
    </Tag>
