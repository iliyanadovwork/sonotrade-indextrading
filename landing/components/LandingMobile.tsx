...
function MonogramBackground({ opacity: op = 0.07 }: { opacity?: number }) {
  ...
  return (
    <div aria-hidden style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {tiles.map((t, i) => (
        <img key={i} src="/st-glyph.png" alt="" style={{ position: 'absolute', left: t.x, top: t.y, width: SZ, height: SZ, opacity: op, userSelect: 'none', pointerEvents: 'none' }}/>
      ))}
    </div>
  )
}
...
export function LandingMobile() {
  ...
  useEffect(() => {
    ...
    Promise.all(HERO_IDS.map(id =>
      fetch(`/api/artist/${encodeURIComponent(id)}?slim=true`).then(r => r.ok ? r.json() : null).then(d => d?.artist ?? null).catch(() => null)
    )).then(results => {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: BG, fontFamily: FONT }}>
      <style>{`
        ...
        .mlnd-btn { display: inline-flex; align-items: center; justify-content: center; border-radius: 9999px; background: ${WHITE}; color: ${BG}; border: none; cursor: pointer; font-family: ${FONT}; font-weight: 500; transition: opacity 0.075s; }
        .mlnd-btn:hover { opacity: 0.8; }
        .mlnd-btn:active { transform: scale(0.92); }
        *, *::before, *::after { box-sizing: border-box; }
        .mlnd-inner  { width: 100%; max-width: ${MAX_W}px; margin: 0 auto; padding-left: ${PX}px; padding-right: ${PX}px; }
        .mlnd-sec    { padding-top: 56px; padding-bottom: 56px; }
        .mlnd-row    { display: flex; flex-direction: column; gap: 40px; }
        .mlnd-grid-3 { display: grid; grid-template-columns: 1fr; gap: 14px; }
        .mlnd-grid-2 { display: grid; grid-template-columns: 1fr; gap: 28px; }
        .mlnd-h1 { font-size: 22px; font-weight: 300; line-height: 1.15; color: ${WHITE}; font-family: ${FONT}; margin: 0 0 16px; padding: 0; }
        .mlnd-h2 { font-size: 21px; font-weight: 300; line-height: 1.2; color: ${WHITE}; font-family: ${FONT}; margin: 0; padding: 0; }
      `}</style>

      {/* ── Hero ── */}
      <section style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="mlnd-inner mlnd-sec">
          <div className="mlnd-row" style={{ alignItems: 'flex-start' }}>
            {/* Text */}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24 }}>
                <img src="/st-glyph.png" alt="" style={{ height: 46, width: 'auto' }}/>
                <span style={{ fontSize: 34, fontWeight: 400, letterSpacing: '-0.05em', color: WHITE, fontFamily: FONT, lineHeight: 1.5 }}>Sonotrade</span>
              </div>
              <h1 className="mlnd-h1">The market where artists become tradable</h1>
              <p style={{ fontSize: 13, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>
                Connecting retail traders, record labels, and institutional participants through a single, data-driven exchange.
              </p>
              <button
                onClick={() => setShowWaitlist(true)}
                style={{ marginTop: 20, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 9999, backgroundColor: WHITE, color: BG, border: 'none', cursor: 'pointer', fontFamily: FONT, fontWeight: 500, fontSize: 14, padding: '8px 20px', transition: 'opacity 0.075s' }}
            ...
            <div style={{ flex: 1, minWidth: 0, width: '100%', minHeight: 352 }}>
              {!heroLoaded ? (
                /* Transparent spacer — reserves height without showing skeleton UI */
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                    ...
                    {changeData && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <svg width="14" height="14" viewBox="0 0 24 18" fill="none" style={{ color: chartColor, transform: `rotate(${changeData.percentChange >= 0 ? '0deg' : '180deg'}) translateY(${changeData.percentChange >= 0 ? '0px' : '3px'})`, marginTop: changeData.percentChange >= 0 ? 3 : 0 }}>
                          <path fill="currentColor" d="m12 0 10.392 14.25H1.608z"/>
                        </svg>
      ...
      <section id="how-it-works" style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="mlnd-inner mlnd-sec" style={{ paddingBottom: 16 }}>
      ...
      <section id="ar-team" style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="mlnd-inner mlnd-sec">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 40 }}>
            <h2 className="mlnd-h2">An A&amp;R engine built on market behaviour, not traditional scouting</h2>
      ...
      {/* ── Enterprise API ── */}
      <section style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="mlnd-inner" style={{ paddingTop: 64, paddingBottom: 64 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32, padding: 24, borderRadius: 16, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(255,255,255,0.02)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
              ...
              <h2 className="mlnd-h2" style={{ marginBottom: 12 }}>Programmatic access to Sonotrade&apos;s market data</h2>
              <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={row.endpoint} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingLeft: 14, paddingRight: 14, paddingTop: 10, paddingBottom: 10, borderRadius: 8, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(10,10,10,0.8)', overflow: 'hidden' }}>
                  <code style={{ fontSize: 11, color: WHITE, fontFamily: MONO, opacity: 0.85, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.endpoint}</code>
                  <span style={{ fontSize: 10, flexShrink: 0, color: SEC, fontFamily: FONT, paddingLeft: 8 }}>{row.desc}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ borderTop: `1px solid ${BORDER}`, backgroundColor: BG, position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="mlnd-inner" style={{ paddingTop: 120, paddingBottom: 120, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 20, position: 'relative', zIndex: 1 }}>
          <h2 className="mlnd-h1" style={{ maxWidth: 640, margin: 0 }}>The Market Infrastructure for Music. Coming Soon.</h2>
          ...
          <button
            onClick={() => setShowWaitlist(true)}
            style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 9999, backgroundColor: WHITE, color: BG, border: 'none', cursor: 'pointer', fontFamily: FONT, fontWeight: 500, fontSize: 14, padding: '8px 20px', transition: 'opacity 0.075s' }}
            onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.opacity = '0.8' }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.opacity = '1' }}
          >
            Join Waitlist
