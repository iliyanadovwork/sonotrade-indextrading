...
function AboutChart({
  ...
  // Use toFixed(2) to match artist page precision — reduces rounding gap vs computed totalLen
  ...
  // Clip x boundary grows left→right as progress goes 0→1 (x = time, guaranteed chronological)
  const clipW   = W * progress;
  const clipId  = `lnd-clip-${dataKey.replace(/[^a-z0-9]/gi, '-')}`;

  // Dot: find point on path at current clip boundary x
  let dotPos = last ?? { x: 0, y: 0 };
  if (!done && progress > 0 && points.length > 1) {
    ...
    for (let i = 1; i < points.length; i++) {
      if (points[i].x >= targetX) {
        ...
        dotPos = { x: targetX, y: prev.y + st * (curr.y - prev.y) };
        break;
      }
    }
  }
...
function StreamingMetricsScroller() {
  ...
  return (
    <div style={{ width: '100%', maxWidth: 560, height: CONTAINER_H, perspective: 900, perspectiveOrigin: '50% 50%', overflow: 'hidden', position: 'relative', maskImage: 'linear-gradient(to bottom,transparent 0%,black 30%,black 70%,transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom,transparent 0%,black 30%,black 70%,transparent 100%)' }}>
      <div ref={innerRef} style={{ position: 'relative', height: '100%', transformStyle: 'preserve-3d' }}>
        {METRIC_ROWS.map(item => (
          <div key={item.source} data-row style={{ position: 'absolute', left: 0, right: 0, height: ROW_H, opacity: 0, transformOrigin: '50% 50%' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 16, paddingRight: 16, height: '100%', borderRadius: 8, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(255,255,255,0.02)', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                <span style={{ fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: SEC, fontFamily: FONT, flexShrink: 0 }}>{item.source}</span>
                <span style={{ fontSize: 13, color: WHITE, fontFamily: FONT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.label}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                <span style={{ fontSize: 11, color: SEC, fontFamily: FONT, display: 'none' } as any} className="lnd-scroller-note">{item.note}</span>
                <span style={{ fontSize: 11, fontWeight: 500, color: item.trend.startsWith('+') ? POSITIVE : NEGATIVE, fontFamily: FONT }}>{item.trend}</span>
                <span style={{ fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '3px 6px', borderRadius: 4, border: `1px solid ${BORDER}`, fontFamily: FONT, color: item.signal === 'HIGH' ? WHITE : SEC, backgroundColor: item.signal === 'HIGH' ? 'rgba(255,255,255,0.08)' : 'transparent' }}>{item.signal}</span>
              </div>
            </div>
          </div>
        ))}
...
// ── CultureNetworkGraphic (responsive SVG) ─────────────────────────────────
function CultureNetworkGraphic() {
  return (
    <svg width="100%" viewBox="0 0 500 500" style={{ maxWidth: 500, display: 'block', overflow: 'visible' }}>
      <defs>
        <path id="lc1" d="M 102 102 L 250 250"/><path id="lc2" d="M 398 102 L 250 250"/>
        <path id="lc3" d="M 102 398 L 250 250"/><path id="lc4" d="M 398 398 L 250 250"/>
        <path id="lc5" d="M 250 40 L 250 250" /><path id="lc6" d="M 460 250 L 250 250"/>
        <path id="lc7" d="M 250 460 L 250 250"/><path id="lc8" d="M 40 250 L 250 250" />
      </defs>
      ...
      {(['lc1','lc2','lc3','lc4','lc5','lc6','lc7','lc8'] as const).map((id,i)=>(
...
function RadarGraphic() {
  return (
    <svg width="100%" viewBox="0 0 460 460" style={{ maxWidth: 460, display: 'block', overflow: 'visible' }}>
      <defs>
...
function MonogramBackground({ opacity: op = 0.07 }: { opacity?: number }) {
  const glyphSrc = require('@/assets/images/st-glyph.png') as any;
  const src = typeof glyphSrc === 'string' ? glyphSrc : (glyphSrc?.default ?? glyphSrc?.uri ?? '');
  ...
  return (
    <div aria-hidden style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      {tiles.map((t, i) => (
        <img key={i} src={src} alt="" style={{ position: 'absolute', left: t.x, top: t.y, width: SZ, height: SZ, opacity: op, userSelect: 'none', pointerEvents: 'none' }}/>
      ))}
    </div>
  );
}
...
// ── TopArtistsList ─────────────────────────────────────────────────────────
interface TopArtist { id: string; spotify_id?: string | null; name: string; current_index_value: number | null; change_1m: number | null; image_url?: string | null }
function TopArtistsList({ onArtistClick }: { onArtistClick?: (id: string) => void }) {
  const [artists, setArtists] = useState<TopArtist[]>([]);
  const [idx, setIdx]         = useState(0);

  useEffect(() => {
    fetch(`${ENDPOINTS.ARTISTS}?limit=10`)
      .then(r => r.json()).then(d => setArtists(d.artists ?? d ?? [])).catch(() => {});
  }, []);
  ...
  return (
    <div key={`${artist.id}-${idx}`} style={{ animation: 'artistFadeIn 0.4s cubic-bezier(0.25,0.46,0.45,0.94) both', marginTop: 0 }}>
      <button type="button" onClick={() => artistId && onArtistClick?.(artistId)}
        style={{ width: '100%', background: 'none', border: 'none', padding: 0, cursor: 'pointer', outline: 'none', textAlign: 'left', display: 'block' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 10, border: `1px solid ${BORDER}`, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.02)' }}>
          {artist?.image_url
            ? <img src={artist.image_url} alt={artist.name} style={{ width: 30, height: 30, flexShrink: 0, borderRadius: '50%', objectFit: 'cover' }}/>
            : <div style={{ width: 30, height: 30, flexShrink: 0, borderRadius: '50%', backgroundColor: '#27272a' }}/>}
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 1 }}>
            <span style={{ fontSize: 13, color: WHITE, fontFamily: FONT, fontWeight: 400, letterSpacing: '-0.2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>{artist?.name}</span>
            <span style={{ fontSize: 10, color: SEC, fontFamily: FONT }}>Index</span>
          </div>
          <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
            <span style={{ fontSize: 11, color: SEC, fontFamily: FONT }}>
              {artist?.current_index_value != null ? artist.current_index_value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'}
              <span style={{ fontSize: 9, marginLeft: 3 }}>USD</span>
            </span>
            {change != null && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                <svg width="10" height="10" viewBox="0 0 24 14" fill="none" style={{ color: change >= 0 ? POSITIVE : NEGATIVE, transform: `rotate(${change >= 0 ? 0 : 180}deg)`, flexShrink: 0, marginTop: 2 }}>
                  <path fill="currentColor" d="m12 0 10.392 14.25H1.608z"/>
                </svg>
                <span style={{ fontSize: 11, fontWeight: 500, color: change >= 0 ? POSITIVE : NEGATIVE, fontFamily: FONT }}>{Math.abs(change).toFixed(2)}%</span>
              </div>
            )}
          </div>
        </span>
...
// ── FoundationsGraphic ─────────────────────────────────────────────────────
const F_CX = [82, 214, 346, 478]; // circle centers — evenly spaced in 560-wide viewBox
const F_CY = 28, F_R = 22;

function FoundationsGraphic({ onArtistClick }: { onArtistClick?: (id: string) => void }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
      <svg width="100%" viewBox="0 0 560 258" style={{ overflow: 'visible' }}>
        <defs>
          <path id="p1" d={`M ${F_CX[0]} ${F_CY+F_R} L ${F_CX[0]} 140 L 280 200 L 280 258`}/>
          <path id="p2" d={`M ${F_CX[1]} ${F_CY+F_R} L ${F_CX[1]} 140 L 280 200 L 280 258`}/>
          <path id="p3" d={`M ${F_CX[2]} ${F_CY+F_R} L ${F_CX[2]} 140 L 280 200 L 280 258`}/>
          <path id="p4" d={`M ${F_CX[3]} ${F_CY+F_R} L ${F_CX[3]} 140 L 280 200 L 280 258`}/>
        </defs>
        {/* Circles */}
        ...
        {/* Vertical lines from bottom of each circle */}
      ...
      <div style={{ width: '100%', paddingLeft: 28, paddingRight: 28, marginTop: 0 }}>
        <TopArtistsList onArtistClick={onArtistClick}/>
      </div>
    </div>
  );
}
...
function SectionLabel({ text }: { text: string }) {
  return <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: SEC, fontFamily: FONT }}>{text}</span>;
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      {items.map(({ title, body }) => (
        <div key={title}>
          <h3 style={{ margin: '0 0 10px', padding: 0, fontSize: 14, fontWeight: 400, color: WHITE, fontFamily: FONT }}>{title}</h3>
          <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>{body}</p>
        </div>
      ))}
    </div>
  );
...
export default function LandingScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all(HERO_IDS.map(id =>
      fetch(ENDPOINTS.ARTIST(id)).then(r => r.ok ? r.json() : null).then(d => d?.artist ?? null).catch(() => null)
    )).then(results => {
      if (!cancelled) { setHeroArtists(results.filter(Boolean) as any[]); setHeroLoaded(true); }
    });
    return () => { cancelled = true; };
  }, []);

  useEffect(() => {
    if (!heroLoaded || !heroArtists.length) return;
    const t = setInterval(() => {
      setHeroOpacity(0);
      setTimeout(() => {
  ...
  const handleStartTrading = () => router.push('/welcome');
  const handleArtistClick  = useCallback((id: string) => router.push(`/artist/${id}` as any), [router]);

  const glyphSrc = require('@/assets/images/st-glyph.png') as any;
  const glyphUri = typeof glyphSrc === 'string' ? glyphSrc : (glyphSrc?.default ?? glyphSrc?.uri ?? '');

  const PX = 20; // mobile side padding
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: BG, fontFamily: FONT }}>
      <style>{`
        ...
        /* nav links */
        .lnd-nav-link { color: ${SEC}; text-decoration: none; font-size: 13px; opacity: 0.85; letter-spacing: -0.02em; transition: opacity 0.15s; }
        .lnd-nav-link:hover { opacity: 1; }
        /* buttons */
        .lnd-btn { display: inline-flex; align-items: center; justify-content: center; border-radius: 9999px; background: ${WHITE}; color: ${BG}; border: none; cursor: pointer; font-family: ${FONT}; font-weight: 500; transition: opacity 0.075s; }
        .lnd-btn:hover { opacity: 0.8; }
        .lnd-btn:active { transform: scale(0.92); }
        .lnd-btn-sm  { font-size: 13px; padding: 7px 18px; }
        .lnd-btn-md  { font-size: 14px; padding: 10px 24px; }
        .lnd-btn-lg  { font-size: 15px; padding: 13px 36px; }
        /* layout helpers */
        *, *::before, *::after { box-sizing: border-box; }
        .lnd-inner  { width: 100%; max-width: ${MAX_W}px; margin: 0 auto; padding-left: ${PX}px; padding-right: ${PX}px; }
        .lnd-sec    { padding-top: 56px; padding-bottom: 56px; }
        .lnd-row    { display: flex; flex-direction: column; gap: 40px; }
        .lnd-graphic-wrap { display: flex; justify-content: center; width: 100%; }
        /* grid helpers */
        .lnd-grid-3 { display: grid; grid-template-columns: 1fr; gap: 14px; }
        .lnd-grid-2 { display: grid; grid-template-columns: 1fr; gap: 28px; }
        /* note in scroller — hidden on mobile */
        .lnd-note { display: none; }
        /* hero title */
        .lnd-h1 { font-size: 22px; font-weight: 300; line-height: 1.15; color: ${WHITE}; font-family: ${FONT}; margin: 0 0 16px; padding: 0; }
        .lnd-h2 { font-size: 21px; font-weight: 300; line-height: 1.2; color: ${WHITE}; font-family: ${FONT}; margin: 0; padding: 0; }
        /* desktop overrides */
        @media (min-width: 900px) {
          .lnd-btn-sm { font-size: 14px; padding: 8px 20px; }
          .lnd-sec    { padding-top: 100px; padding-bottom: 100px; }
          .lnd-row    { flex-direction: row; align-items: flex-start; gap: 80px; }
          .lnd-row-center { align-items: center; }
          .lnd-row-reverse { flex-direction: row-reverse; }
          .lnd-graphic-wrap { justify-content: flex-start; }
          .lnd-graphic-foundations { width: 560px; flex-shrink: 0; }
          .lnd-graphic-network     { width: 500px; flex-shrink: 0; }
          .lnd-graphic-data        { width: 460px; flex-shrink: 0; }
          .lnd-graphic-scroller    { flex-shrink: 0; display: flex; justify-content: flex-end; }
          .lnd-grid-3  { grid-template-columns: repeat(3, 1fr); gap: 20px; }
          .lnd-grid-2  { grid-template-columns: repeat(2, 1fr); gap: 48px 64px; }
          .lnd-note    { display: inline; }
          .lnd-h1  { font-size: 36px; }
          .lnd-h2  { font-size: 30px; }
          .lnd-inner { padding-left: 32px; padding-right: 32px; }
        }
        @media (min-width: 1100px) {
          .lnd-h1 { font-size: 42px; }
        }
      `}</style>

      {/* ── Hero ── */}
      <section style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec">
          <div className="lnd-row lnd-row-center">
            {/* Text */}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 0, marginBottom: 24 }}>
                {glyphUri && <img src={glyphUri} alt="" style={{ height: 46, width: 'auto' }}/>}
                <span style={{ fontSize: 34, fontWeight: 400, letterSpacing: '-0.05em', color: WHITE, fontFamily: FONT, lineHeight: 1.5 }}>Sonotrade</span>
              </div>
              <h1 className="lnd-h1">The market where artists become tradable</h1>
              <p style={{ fontSize: 13, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: '0 0 24px' }}>
                Connecting retail traders, record labels, and institutional participants through a single, data-driven exchange.
              </p>
              <button className="lnd-btn lnd-btn-md" onClick={handleStartTrading}>Start Trading</button>
            </div>
            {/* Chart */}
            {heroLoaded && artist && (
              <div style={{ flex: 1, minWidth: 0, width: '100%', opacity: heroOpacity, transition: 'opacity 0.6s ease' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  {artist.image_url
                    ? <img src={artist.image_url} alt={artist.name} style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} draggable={false}/>
                    : <div style={{ width: 48, height: 48, borderRadius: '50%', backgroundColor: BORDER, flexShrink: 0 }}/>}
                  <h2 style={{ margin: 0, padding: 0, fontSize: 22, fontWeight: 300, color: WHITE, fontFamily: FONT }}>{artist.name ?? ''}</h2>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 26, fontWeight: 600, color: WHITE, fontFamily: FONT, letterSpacing: '-0.02em' }}>
                    ${(drawingPrice ?? artist.index_price ?? 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                  <span style={{ fontSize: 18, fontWeight: 300, color: WHITE, fontFamily: FONT }}>points</span>
                  {changeData && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <svg width="14" height="14" viewBox="0 0 24 18" fill="none" style={{ color: chartColor, transform: `rotate(${changeData.percentChange >= 0 ? 0 : 180}deg)`, marginTop: 3 }}>
                        <path fill="currentColor" d="m12 0 10.392 14.25H1.608z"/>
                      </svg>
                      <span style={{ fontSize: 14, color: chartColor, fontFamily: FONT }}>{Math.abs(changeData.percentChange).toFixed(2)}%</span>
                      <span style={{ fontSize: 14, color: chartColor, fontFamily: FONT }}>
                        {changeData.rawChange >= 0 ? '+' : '-'}${Math.abs(changeData.rawChange).toFixed(2)}
                      </span>
                    </div>
                  )}
                </div>
                <AboutChart data={chartData} height={220} onPrice={setDrawingPrice} onChange={setChangeData} onColor={setChartColor}/>
              </div>
            )}
          </div>
        </div>
      ...
      <section id="foundations" style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec">
          <div className="lnd-row">
            {/* Text first on mobile */}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ marginBottom: 40 }}><SectionLabel text="WHAT IS SONOTRADE"/></div>
              <TextBlocks items={[
            ...
            {/* Graphic below on mobile, left on desktop */}
            <div className="lnd-graphic-foundations lnd-graphic-wrap">
              <div style={{ width: '100%' }}>
                <FoundationsGraphic onArtistClick={handleArtistClick}/>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="how-it-works" style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec" style={{ paddingBottom: 16 }}>
          <div style={{ marginBottom: 40 }}><SectionLabel text="WHY STREAMING METRICS MATTER"/></div>
          <div className="lnd-row">
            <div style={{ flex: 1, minWidth: 0 }}>
              <TextBlocks items={[
            ...
            <div className="lnd-graphic-scroller lnd-graphic-wrap">
              <StreamingMetricsScroller/>
            </div>
          </div>
        </div>
      ...
      <section style={{ backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec">
          <div className="lnd-row">
            {/* Text */}
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ marginBottom: 40 }}><SectionLabel text="WHO IS SONOTRADE FOR"/></div>
              <div className="lnd-grid-2">
                {[
                  ...
                  <div key={num} style={{ display: 'flex', flexDirection: 'column', gap: 10, ...(span2 ? { gridColumn: '1 / -1' } : {}) }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      <span style={{ fontSize: 36, fontWeight: 300, color: WHITE, fontFamily: FONT, lineHeight: 1 }}>{num}</span>
                      {icon}
                    </div>
                    <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>{body}</p>
                  </div>
            ...
            {/* Network graphic below on mobile */}
            <div className="lnd-graphic-network lnd-graphic-wrap">
      ...
      <section id="data-research" style={{ borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec">
          <div className="lnd-row">
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ marginBottom: 40 }}><SectionLabel text="DATA & RESEARCH"/></div>
              <TextBlocks items={[
            ...
            <div className="lnd-graphic-data lnd-graphic-wrap">
              <RadarGraphic/>
            </div>
          </div>
        </div>
      ...
      <section id="ar-team" style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner lnd-sec">
          <div style={{ marginBottom: 40 }}><SectionLabel text="A&R TEAM"/></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 40 }}>
            <h2 className="lnd-h2">An A&amp;R engine built on market behaviour, not traditional scouting</h2>
            <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>
          ...
          <div className="lnd-grid-3">
            {[
              ...
              <div key={num} style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 18, borderRadius: 12, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(255,255,255,0.02)' }}>
                <span style={{ fontSize: 36, fontWeight: 300, color: WHITE, fontFamily: FONT, lineHeight: 1 }}>{num}</span>
                <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>{body}</p>
              </div>
            ))}
      ...
      {/* ── Enterprise API ── */}
      <section style={{ borderBottom: `1px solid ${BORDER}`, backgroundColor: BG }}>
        <div className="lnd-inner" style={{ paddingTop: 64, paddingBottom: 64 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32, padding: 24, borderRadius: 16, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(255,255,255,0.02)', position: 'relative', overflow: 'hidden' }}>
            <div aria-hidden style={{ position: 'absolute', inset: 0, backgroundImage: `linear-gradient(${BORDER} 1px, transparent 1px), linear-gradient(90deg, ${BORDER} 1px, transparent 1px)`, backgroundSize: '48px 48px', opacity: 0.3, zIndex: 0 }}/>
            <div style={{ position: 'relative', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: SEC, fontFamily: FONT }}>ENTERPRISE API</span>
                <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', padding: '2px 8px', borderRadius: 4, color: SEC, border: `1px solid ${BORDER}`, fontFamily: FONT, backgroundColor: 'rgba(255,255,255,0.04)' }}>Coming soon</span>
              </div>
              <h2 className="lnd-h2" style={{ marginBottom: 12 }}>Programmatic access to Sonotrade's market data</h2>
              <p style={{ fontSize: 12, color: SEC, fontFamily: FONT, lineHeight: 1.7, margin: 0 }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <div key={row.endpoint} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingLeft: 14, paddingRight: 14, paddingTop: 10, paddingBottom: 10, borderRadius: 8, border: `1px solid ${BORDER}`, backgroundColor: 'rgba(10,10,10,0.8)' }}>
                  <code style={{ fontSize: 11, color: WHITE, fontFamily: MONO, opacity: 0.85 }}>{row.endpoint}</code>
                  <span style={{ fontSize: 10, flexShrink: 0, color: SEC, fontFamily: FONT }}>{row.desc}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section style={{ borderTop: `1px solid ${BORDER}`, backgroundColor: BG, position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="lnd-inner" style={{ paddingTop: 120, paddingBottom: 120, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 20, position: 'relative', zIndex: 1 }}>
          <h2 className="lnd-h1" style={{ maxWidth: 640, margin: 0 }}>The Market Infrastructure for Music. Now in beta.</h2>
          <p style={{ fontSize: 13, color: SEC, fontFamily: FONT, lineHeight: 1.6, margin: 0 }}>Real data. Real positions. The first exchange for music derivatives.</p>
          <button className="lnd-btn lnd-btn-lg" style={{ marginTop: 8 }} onClick={handleStartTrading}>Start Trading</button>
        </div>
      </section>
    </div>
  );
